...
# Requires: all_transactions list of Transaction input objects (incomes and expenses)
# Modifies: Event, Timeline classes
# Effects: Timeline.create_timeline() creates a timeline of events, and
# periodic_events.
...
class Event(object):
    ...
    def print_event(self):
        ''' prints event name, date, and amount 
        '''
...
def total(events):
    ''' since expense amounts are negative, returns net amount 
    '''
    return sum(event.amount for event in events)


def is_solvent(events):
    ''' returns true if overall solvent 
    '''
    if total(events) > 0:
        return True
    else:
...
class Timeline(object):
    ...
    @classmethod
    def create_monthly_events(self, all_transactions):
        ''' returns events with newly created monthly events 
        '''
        events = []
        for transaction in all_transactions:
            ...
            if transaction.schedule.start != 0:
                months = months_between(
                    transaction.schedule.start, Timeline.default_end)
            else:
                months = months_between(
                    Timeline.default_start, Timeline.default_end)
            if transaction.get_frequency() == model.Frequency.MONTHLY:
                for month in months:
                    for day in transaction.schedule.days:
                        date = dt.date(2016, month, day)
                        new_event = Event(
                            transaction.name, transaction.amount, date, income_type=transaction.income_type)
                        events.append(new_event)
        return events

    @classmethod
    def create_interval_events(self, all_transactions):
        """ returns events with newly created interval events 
        """
        events = []
        for transaction in all_transactions:
            if transaction.get_frequency() == model.Frequency.INTERVAL:
                ...
                while (current_date < Timeline.default_end):
                    new_event = Event(transaction.name, transaction.amount,
                                      current_date, income_type=transaction.income_type)
                    events.append(new_event)
                    current_date += timedelta(days=transaction.schedule.period)
        return events

    @classmethod
    def create_onetime_events(self, all_transactions):
        """ returns events with newly created onetime events 
        """
        events = []
        for transaction in all_transactions:
            if transaction.get_frequency() == model.Frequency.ONE_TIME:
                new_event = Event(transaction.name, transaction.amount,
                                  transaction.schedule.start, income_type=transaction.income_type)
                events.append(new_event)
        return events

    @classmethod
    def assemble_events(self, all_transactions):
        """ returns all newly created events 
        """
        monthly_events = Timeline.create_monthly_events(all_transactions)
        interval_events = Timeline.create_interval_events(all_transactions)
        onetime_events = Timeline.create_onetime_events(all_transactions)

        events = monthly_events + interval_events + onetime_events
    ...
    def create_periods(self, events):
        """ returns periodic_events, chunked by primary income 
        """
        new_group = []
        periodic_events = []
    ...
    def output_timeline(self):
        """ outputs timeline events as JSON 
        """
        data = {}
        events = []
        for event in self.events:
...
def months_between(start, end):
    """ returns list of months between two dates 
    """
    months = []
    cursor = start
    while cursor <= end:
